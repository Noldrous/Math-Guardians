...
class TileMap:
    def __init__(self):
        ...
        # TINANGGAL ANG YELLOW (255,255,0)
    ...
    def handle_click(self, pos):
        if self.holding_color != None:
            ...
            if pos[1] >= box.top and pos[1] <= box.bottom:
                if pos[0] > box.right and pos[0] <= 500:
                    self.placed_towers.append([pos[0]-20, pos[1]-20, self.holding_color])
        ...
        if self.show_menu == True:
            box = self.boxes[self.selected_box_index]
            # PINALITAN ANG range(4) NG len(self.colors)
    ...
    def draw(self, screen):
        ...
        for tower in self.placed_towers:
            pygame.draw.rect(screen, tower[2], (tower[0], tower[1], 40, 40))
